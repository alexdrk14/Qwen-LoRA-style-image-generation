...
def main():
    ...
    if hasattr(args, 'resume_from_checkpoint') and args.resume_from_checkpoint:
        if args.resume_from_checkpoint == "latest":
            ...
            if dirs:
                ...
                logger.info(f"Resuming from checkpoint: {resume_from_checkpoint} at step {global_step}")
            else:
                logger.info(f"No checkpoints found in {args.output_dir}, starting from scratch")
        else:
    ...
    #LOAD CHECKPOINT WEIGHTS AFTER PREPARE
    if resume_from_checkpoint:
        logger.info(f"Loading LoRA weights from {resume_from_checkpoint}")
        lora_state_dict = QwenImagePipeline.lora_state_dict(resume_from_checkpoint)
        unwrapped_model = accelerator.unwrap_model(flux_transformer)
        incompatible_keys = unwrapped_model.load_state_dict(lora_state_dict, strict=False)
        logger.info(f"Loaded LoRA checkpoint. Missing keys: {len(incompatible_keys.missing_keys)}, Unexpected keys: {len(incompatible_keys.unexpected_keys)}")
    ...
    logger.info(f"  Starting from global step = {initial_global_step}")
    
    progress_bar = tqdm(
        range(0, args.max_train_steps),  #  Always 0 to max_train_steps
